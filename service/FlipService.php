...
require_once('repo/FlipRepo.php');
...
class FlipService { 
	...
	public function doPost($param){
		...
		if(count($dataResponse) > 2){
			$this->FlipRepo->insertResponse($dataResponse);
		}
	...
	public function doCheck($id,$transactions_id){
		...
		//Validasi response
		if(count($dataResponse) > 2){
			$this->FlipRepo->updateResponse($id,$dataResponse);
		}
    ...
    public function showAllData(){
    	return $this->FlipRepo->showAllData();
    }
